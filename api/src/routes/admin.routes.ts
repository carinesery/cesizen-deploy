...
router.post("/users", authMiddleware, roleMiddleware(UserRoleEnum.ADMIN), upload.single("profilPictureUrl"), validate(adminRegisterSchema, "body"), adminCreateUserController);
router.patch("/users/:id", authMiddleware, roleMiddleware(UserRoleEnum.ADMIN), upload.single("profilPictureUrl"), validate(adminUpdateUserParamsSchema, "params"), validate(adminUpdateUserBodySchema, "body"), updateUserController);
